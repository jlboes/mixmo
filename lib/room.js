...
Room.dispatchLetters = function(idRoom, nb_letters){
    ...
    for(var i=0,len=mPlayers.length; i<len; i++){
      ...
      Meteor.log.debug(" Room.dispatchLetters | userId : " + p.id+" --> " + p.name);
      Meteor.log.debug(currentletters[p.id]);
    }
...
Room.handleMixmo = function(idRoom, idUser){
  Meteor.log.debug("handleMixmo | room " + idRoom);
  ...
  if(mRoom.letters.length < 2 * mPlayers.length){
    ...
    // So act accordingly
    Meteor.log.debug("handleMixmo | room " + idRoom+", useId : "+idUser+", Not enough letters to dispatch --> game End");
    Rooms.update(idRoom, {
      $set: {
    ...
    Meteor.log.debug("handleMixmo | room " + idRoom+" --> will dispatch.");
...
Room.addCurrentLetter = function(idRoom, letter){
  Meteor.log.debug("Room.addCurrentLetter | userId : " + Meteor.userId());
  var mRoom = Rooms.findOne(idRoom);
...
Room.removeCurrentLetter = function(idRoom, letter){
  Meteor.log.debug("Room.removeCurrentLetter | userId : " + Meteor.userId());
  var mRoom = Rooms.findOne(idRoom);
...
Room.moveGridletter =  function(idRoom, letter, newcoords) {
  Meteor.log.debug("Room.moveGridletter | userId : " + Meteor.userId());
  var mRoom = Rooms.findOne(idRoom);
...
Room.reset = function(idRoom){
  console.log("Room.reset | idRoom : "+idRoom+", userId : " + Meteor.userId());
  var mRoom = Rooms.findOne(idRoom);
